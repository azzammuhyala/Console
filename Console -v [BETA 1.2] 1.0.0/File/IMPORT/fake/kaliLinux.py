...
if name_kali.isidentifier():
  ...
  while True:
    join_dirs = '/'.join(dirs_name_to)
    add_slash = '/' if join_dirs else ''
    kali_inp = input(f"\033[36m┌──(\033[34m{name_kali}㉿{device_kali}\033[36m)-[\033[0m~{add_slash + join_dirs}\033[36m]\n└─$ \033[0m")
    if kali_inp:
      if kali_inp == "exit":
        break
      elif kali_inp == "clear":
...
A star (*) next to a name means that the command is disabled.
 ...
 help [-dms] [pattern ...]                                                                                               { COMMANDS ; }""")
  ...
  print(f"Your Name Kali. (Original: kali): \033[36m{name_kali}\033[31m\nerror: Invalid Character.\033[0m")